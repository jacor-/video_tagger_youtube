...
# Given a video ID we check if the frames alerady exist.
#  - If they exist in the proper folder and we have enough frames for the video, we do not download anything
#  - Otherwise, we download the video and extract the required frames.
...
class YoutubeVideoCollector(object):
    def __init__(self, api_key, experiment_name, search_criteria = [], videos_per_search = 50, frames_per_video = 20, save_final_dataset_version = True, min_ocs_tags = 50, max_ocs_tags = 500):
        '''
...
def example_youtube_videos():
    videos_per_search = 2
    frames_per_video = 5

    api_key = st.youtube_api_key
    search_criteria = ["messi neymar suarez", "ronado benzema bale"]
    youtube_videos = YoutubeVideoCollector(api_key, "youtube_small_experiment", search_criteria = search_criteria, videos_per_search = videos_per_search, frames_per_video = frames_per_video, min_ocs_tags = 0, max_ocs_tags=1000)


    for video_id in youtube_videos.get_available_videos():
        print("-->  video id: '%s'" % video_id)
        print("      - frames: %s" % str(youtube_videos.get_frames(video_id)))
        print("      - labels: %s" % str(youtube_videos.get_labels(video_id)))
        print("      - tags: %s" % str(map(lambda x: youtube_videos.vocab_manager.label_to_word[x], youtube_videos.get_labels(video_id))))
