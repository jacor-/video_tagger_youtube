...
###
class CIFAR10OriginalDataset(object):
    @staticmethod
    ...
    def load_data(dataset):
        ...
        meta = numpy.load(open('%s/batches.meta'%dataset,'rb'), encoding ='latin1')['label_names']
        ims, labs = [], []
        for i_batch in range(1,6):
            a,b = getImagesAndLabels(numpy.load(open('%s/data_batch_%d'%(dataset, i_batch),'rb'), encoding ='latin1'))
            ims.append(a)
            labs.append(b)
        train_set = (np.concatenate(ims), np.concatenate(labs))
        test_set  = getImagesAndLabels(numpy.load(open('%s/test_batch'%dataset,'rb'), encoding ='latin1'))
    ...
    def __init__(self):
        CIFAR10OriginalDataset.download_dataset(st.path_cifar_images + "/cifar-10-python.tar.gz")
        train_set, test_set, meta = CIFAR10OriginalDataset.load_data(st.path_cifar_images + "/cifar-10-batches-py")

        test_set_x, test_set_y = CIFAR10OriginalDataset.shared_dataset((test_set[0], CIFAR10OriginalDataset.oneHotEncoding(test_set[1])))
        train_set_x, train_set_y = CIFAR10OriginalDataset.shared_dataset((train_set[0], CIFAR10OriginalDataset.oneHotEncoding(train_set[1])))
